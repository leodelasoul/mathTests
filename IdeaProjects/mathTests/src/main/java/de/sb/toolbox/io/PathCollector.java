...
/**
 * A basic path visitor that can be used to walk a file tree recursively, and collect all
 * directories and regular files encountered. The visitor will not follow that are not symbolic
 * links, or only reachable by a symbolic link. The resulting paths can be accessed after walking a
 * file tree. Note that instances are not thread safe. However, they can be used subsequently for
 * more than one file tree walk.
...
public class PathCollector extends SimpleFileVisitor<Path> {
	...
	/**
	 * Returns the directory paths visited since the collector was created, or the collection
	 * returned was cleared the time.
	...
	/**
	 * Returns the file paths visited since the collector was created, or the collection returned
	 * was cleared the time.
