...
/**
 * This facade simplifies object serialization/deserialization, and cloning of serializable classes.
 * Additionally, this it modifies object stream I/O exception handling to adjust object
 * serialization to modern exception handling standards - therefore causing API use to be more
 * streamlined, more consistent, and less error prone. This is achieved by eliminating all checked
 * I/O exceptions that cannot happen in this design, and additionally by retrowing all checked
 * exceptions that usually point to programming or algorithmic errors as unchecked exceptions.<br />
 * Note that modern design makes very selective and careful use of checked exceptions. An exception
 * should be checked only if BOTH of the following apply:
 * <ul>
 * <li>The problem is usually not caused by programming error or reasons unrelated to the code.
 * Consider why NullPointerException or OutOfMemoryError are unchecked.</li>
 * <li>There is a good chance the problem can be meaningfully remedied in a catch block, i.e. in
 * other ways than simply displaying the problem to an end user. Consider why the more modern JAX-WS
 * API (WebServices) does NOT force any service method to declare RemoteException, unlike the older
 * Java-RMI standard.</li>
 * </ul>
 * Also note that this class is declared final because it is a facade, and therefore not supposed to
 * be extended.
 */
...
public final class Serializables {
	...
	/**
	 * Serializes the given objects and returns the resulting data. Note that the given objects may
	 * be null, but not the given variable arguments list.
	 * @param objects the objects
	 * @return the serialized data
	 * @throws NullPointerException if the given variable arguments list is {@code null}
	 * @throws NotSerializableException if one of the given objects, or one of it's direct or
	 *         indirect fields is expected to be serializable, but is not
	 */
	...
	/**
	 * Deserializes the given data and returns the resulting objects. Note that this method
	 * simplifies object stream exception handling by indicating incompatible serialVersionUIDs as
	 * ClassNotFoundException (similar logic to unreadable files causing FileNotFoundException), and
	 * both corrupted serialization headers and unexpected primitive data as
	 ...
	 * @throws IndexOutOfBoundsException if deserialization would cause access of data outside array
	 *         bounds
	 * @throws IllegalArgumentException if the given data does not represent a valid serialized
	 *         sequence of objects
	 * @throws ClassNotFoundException if a class is required that cannot be loaded, or has a
	 *         serialVersionUID incompatible to it's equivalent represented in the data
	 */
	...
	/**
	 * Deserializes the given data and returns the first object contained within it. This is a
	 * convenience method for use with serialized data containing single objects. Note that this
	 * method simplifies object stream exception handling by indicating incompatible
	 * serialVersionUIDs as ClassNotFoundException (similar logic to unreadable files causing
	 * FileNotFoundException), and both corrupted serialization headers and unexpected primitive
	 * data as IllegalArgumentException (because both point to messed up data).
	 * @param data the data
	 * @param offset the offset
	 * @param length the length
