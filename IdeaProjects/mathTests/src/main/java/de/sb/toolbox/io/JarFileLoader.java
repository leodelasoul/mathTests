...
/**
 * Class loader extending it's parent class loader by loading classes from a given jar-file.
 * Stacking multiple instances of this class results in support for multiple jar-files. Instances
 * takes ownership of their jar-files while they exist, and may optionally remove said jar-files
 * after use.
 */
...
public class JarFileLoader extends ClassLoader {
	...
	/**
	 * Creates a new instance that will load classes from it's parent loader, and if not found from
	 * class files within the given jar-file. If delete is {@code true}, the class loader will
	 * delete it's jar-file once it has been read, which is recommended with temporary libraries
	 * because these tend to be notoriously hard to delete after use. Otherwise, the jar-file will
	 * continue to exist after use.
