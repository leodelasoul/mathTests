...
/**
 * HTTP handler class providing HTTP access to basic web resources contained within file systems or
 * code repositories for the Open/Oracle JDK HTTP server.
 */
public abstract class HttpFileHandler implements HttpHandler {
	...
	/**
	 * Returns the handler's life (and synchronized) content type mappings. Note that the resulting
	 * map allows the registration of additional content types after file handler creation.
	...
	/**
	 * Handles the given HTTP exchange by copying the content of it's request path to it's response.
	 * Only GET requests are supported. The request path is interpreted to be relative to the
	 * handler's context directory, all path's outside of this scope are inaccessible. Sets one of
	...
	/**
	 * Handles the given HTTP exchange by copying the content of the given resource's content to the
	 * exchange's response. Note that the given exchange may be closed upon completion, but doesn't
	 * have to because it is closed by the caller anyways. Also note that the operation should
	 * always set a 2xx response code if it expects normal completion, or otherwise throw an
	 * exception.
	 * @param exchange the HTTP exchange
	 ...
	 * Returns the number of bytes copied, and closes neither source nor sink. Note that
	 * {@link SocketException} is treated as a kind of EOF due to to other side terminating the stream.
	...
	/**
	 * Returns a new file handler that provides HTTP access to basic web resources contained within
	 * the current class-path, including the current project and JAR-files.
	 * @param contextPath the context path
	 * @throws NullPointerException if any of the given arguments is {@code null}
	 */
	...
	/**
	 * Returns a new file handler that provides HTTP access to basic web resources contained within a file
	 * system.
	 * @param contextPath the context path
	...
	/**
	 * HTTP file handlers that provide HTTP access to basic web resources contained within any
	 * JAR-file and project within the current class-path.
	 */
	...
	/**
	 * Instances of this class provide HTTP access to basic web resources contained within a file
	 * system.
	 */
