...
/**
 * This facade represents a collection of logical operations on 64bit integer words which are
 * interpreted as bit-arrays. In opposition to {@link java.util.BitSet}, this class is designed
 * as a facade for bit-parallel operations, in order to maximize raw operation speed and
 * flexibility. Note that this class is declared final because it is a facade, and therefore not
 * supposed to be extended.<br />
 * Also note that almost every operation of this facade is available in a flavor that operates on a
 * single 64bit word, and another one that operates on an array of 64bit words. The single-word
 * operations are at least double as fast as the multi-word versions once fully inlined by the JVM,
 * even when using a single word in both cases. This is caused partly by the multi-word methods
 * requiring thorough boundary checks, while the single-word methods can do without! While the core
 * arguments boundaries for single-word operations are {@code [0, 63]} for indices and offsets (
 * {@code [1, 64]} for ranges), any kind of value may actually be passed; the shift operators used
 * for range masking utilize only the least significant {@code 6} bits of any such argument, which
 * automatically "fits" the arguments into their respective bounds. This implies that a single-word
 * operation with {@code index = 0} behaves the same as with {@code 64} or {@code -64} etc, while an
 * operation with {@code range = 64} behaves the same as with {@code 0} or {@code -64}, etc.
 */
...
public final class BitArrays {
	 ...
	 * @return the byte count
	 * @throws IllegalArgumentException if the given byte count is strictly negative, or exceeds the
	 *         maximum number of bits in a long[]
	 */
	...
	/**
	 * Returns {@code true} if the bit at the specified index is {@code 1}, otherwise {@code false}
	 * for {@code 0}.
	 * @param bits the bits
	...
	/**
	 * Returns a copy of the given bits, with the one at the given index set to the given value. The
	 * value being set is {@code 1} for {@code true}, otherwise {@code 0} for {@code false}.
	 * @param bits the bits
	 * @param index the index
	...
	/**
	 * Returns a copy of the given bits, with the ones within the given range set to their
	 * respective opposite values.
	...
	/**
	 * Returns the number of {@code 1}-bits within the given range, i.e. the population count. Note
	 * that the number of {@code 0}-bits can easily be derived by calculating
	 * {@code 64 - cardinality}.
	 * @param bits the bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	...
	/**
	 * Returns the index of the first (lowest) {@code 1}-bit within the given range, or {@code -1}
	 * if there is none. Note that the index of the first {@code 0}-bit can easily be derived by
	 * passing a flipped bit-array.
	 * @param bits the bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	...
	/**
	 * Returns the index of the last (highest) {@code 1}-bit within the given range, or {@code -1}
	 * if there is none. Note that the index of the last {@code 0}-bit can easily be derived by
	 * passing a flipped bit-array.
	 * @param bits the bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	...
	/**
	 * Returns the bit-wise disjunction ({@code OR}) of the given operands, after masking out any
	 * right operand bits outside the given range.
	...
	/**
	 * Returns the bit-wise conjunction ({@code AND}) of the given operands, after masking out any
	 * right operand bits outside the given range.
	 * @param leftBits the left operand bits
	 * @param rightBits the right operand bits
	 * @param offset the offset of the first bit to process
	...
	/**
	 * Returns the bit-wise exclusive disjunction ({@code XOR}) of the given operands, after masking
	 * out any right operand bits outside the given range.
	 * @param leftBits the left operand bits
	 * @param rightBits the right operand bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	...
	/**
	 * Returns the bit-wise exclusive conjunction ({@code XAND}) of the given operands, after
	 * masking out any right operand bits outside the given range.
	...
	/**
	 * Returns {@code true} if the given operands share at least a single {@code 1}-bit at the same
	 * location within the given range, {@code false} otherwise. Note that the anti-intersect can
	 * easily be derived by passing flipped bits.
	 * @param leftBits the left operand bits
	 * @param rightBits the right operand bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	...
	/**
	 * Returns {@code true} if either (or both) of given operands has a {@code 1}-bit in every
	 * respective location within the given range, {@code false} otherwise. Note that the
	 * anti-complement can easily be derived by passing inverted words.
	 * @param leftBits the left operand bits
	 * @param rightBits the right operand bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	...
	/**
	 * Sets the bit at the specified index to the given value. The value being set is {@code 1} for
	 * {@code true}, otherwise {@code 0} for {@code false}.
	 * @param bits the bits
	 * @param index the index
	 * @param value the value
	 * @throws NullPointerException if the given bits are {@code null}
	...
	/**
	 * Returns the number of the given {@code 1}-bits within the given range, i.e. the population
	 * count. Note that the number of {@code 0}-bits can easily be derived by calculating
	 * {@code (words.length * 64L) - cardinality}.
	 * @param bits the bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	 * @return the cardinality within the given range
	...
	/**
	 * Returns {@code true} if the given operands share at least a single {@code 1}-bit at the same
	 * location within the given range, {@code false} otherwise.
	 * @param leftBits the left operand bits
	 * @param rightBits the right operand bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	...
	/**
	 * Returns {@code true} if either (or both) of given operands has a {@code 1}-bit in every
	 * respective location within the given range, {@code false} otherwise.
	 * @param leftBits the left operand bits
	 * @param rightBits the right operand bits
	 * @param offset the offset of the first bit to process
	 * @param range the number of bits to process
	...
	/**
	 * Rotates the words within the given array to the right (i.e. towards higher indices) by the
	 * given offset.
